...
(function (NW) {
  ...
  var FAMILIES = [
    {
      ...
      craft: 'net-weaving and hull-lore'
    },
    {
      ...
      craft: 'stone-cutting and delving'
    },
    {
      ...
      craft: 'star-reading and salt-trade'
    },
    {
      ...
      patro: '{child} of {parent}’s hearth',
      ...
      craft: 'reed-craft and flood-farming'
    },
    {
      ...
      patro: '{child} {parent}sdottir-or-son',
      ...
      craft: 'bone-carving and ice-craft'
    },
    {
      ...
      craft: 'wheat-lore and wainwrighting'
    }
  ];
  ...
  function word(rnd, fam, withCore) {
    var w = pick(rnd, fam.on);
    if (withCore === undefined ? rnd() < 0.5 : withCore) w += pick(rnd, fam.core);
    return w;
  }

  function personName(rnd, fam) {
    return cap(word(rnd, fam, rnd() < 0.35) + pick(rnd, fam.pEnd));
  }

  function placeName(rnd, fam, kind) {
    var w = word(rnd, fam);
    if (rnd() < 0.4) w += pick(rnd, fam.core);
    return cap(w + pick(rnd, SUFFIX[kind] || SUFFIX.plain));
  }

  function clanName(rnd, fam) {
    return cap(word(rnd, fam, false) + pick(rnd, fam.end));
  }
    ...
    if (hn >= 0.004 && hn <= 0.62 && p.slope[c] <= 0.62) {
      ...
      if (score >= 0.58) {
        ...
        site = {
          ...
          culture: cult,
          name: placeName(rnd, cult.fam, kind)
        };
      }
    }
  ...
  function cultureAt(world, wx, wy) {
    ...
    var culture = {
      ...
      values: [fam.values[0], fam.values[1], fam.values[2]]
        .filter(function (_, i2) { return i2 !== ((rnd() * 3) | 0); }),
      craft: fam.craft,
      stats: { relief: mh, moisture: mm, warmth: mt, sea: sea }
    };
  ...
  function buildNation(world, capSite) {
    ...
    var coreName = cap(word(rnd, fam, true) + pick(rnd, fam.end));
  ...
  function buildFaith(world, capSite, rnd) {
    ...
    var deityName = personName(rnd, fam);
  ...
  function buildPeople(world, site, clans, rnd) {
    ...
    for (var c = 0; c < Math.min(2, clans.length); c++) {
      ...
      var elder = {
        name: personName(rnd, fam), clan: clan.name,
        born: year - elderAge,
      ...
      var mid = {
        ...
        born: year - midAge,
        role: pick(rnd, roles),
        line: fam.patro.replace('{child}', '').replace('{parent}', elder.name).trim()
      };
      ...
      if (midAge > 20) {
        ...
        if (yAge > 3) {
          people.push({
            ...
            line: fam.patro.replace('{child}', '').replace('{parent}', mid.name).trim()
          });
        }
      }
    }
  ...
  function buildArtifacts(world, site, nation, clans, founded, rnd) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      out.push({
        name: 'the ' + cap(word(rnd, site.culture.fam, true)) + ' ' + cap(form),
        material: material,
